...
class VendorController extends Controller
{
    ...
    /**
     * Activate a payment gateway account for the store and redirect to onboarding.
     */
    public function activatePaymentAccount(Store $store, string $gateway = 'stripe_connect')
    {
        try {
            ...
            $result = $orchestrator->onboard(
                ...
                gateway: $gateway,
                urls: [
                    'refresh_url' => route('admin.store.view', $store->id),
                    'return_url' => route('admin.store.view', $store->id),
                ],
            );
            ...
            Log::error('VendorController: failed to activate payment account', [
                'store_id' => $store->id,
                'gateway' => $gateway,
                'error' => $e->getMessage(),
            ]);

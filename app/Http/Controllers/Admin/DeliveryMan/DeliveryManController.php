...
class DeliveryManController extends BaseController
{
    ...
    /**
     * Activate a payment gateway account for the delivery man and redirect to onboarding.
     */
    public function activatePaymentAccount(string|int $id, string $gateway = 'stripe_connect')
    {
        $deliveryMan = \App\Models\DeliveryMan::withoutGlobalScope('delivery_only')->findOrFail($id);

        try {
            $orchestrator = app(PartnerPaymentOrchestrator::class);
            $result = $orchestrator->onboard(
                partner: $deliveryMan,
                gateway: $gateway,
                urls: [
                    'refresh_url' => route('admin.users.delivery-man.preview', $deliveryMan->id),
                    'return_url' => route('admin.users.delivery-man.preview', $deliveryMan->id),
                ],
            );
            ...
            Toastr::success(translate('messages.payment_account_already_active'));
        } catch (\Throwable $e) {
            Log::error('DeliveryManController: failed to activate payment account', [
                'delivery_man_id' => $id,
                'gateway' => $gateway,
                'error' => $e->getMessage(),
            ]);
            Toastr::error(translate('messages.failed_to_activate_payment_account'));
        }

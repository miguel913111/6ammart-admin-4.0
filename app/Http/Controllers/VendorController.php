...
public function final_step(Request $request){
    ...
    public function stripeOnboardingReturn(Request $request)
    {
        $store = Store::find($request->store_id);
        $accountId = $store?->gateway_account_id ?? $store?->stripe_account_id ?? null;

        if ($store && !empty($accountId)) {
            try {
                $stripeService = new StripeConnectService();
                $status = $stripeService->retrieveAccount($accountId);

                $store->gateway_account_status = ($status['charges_enabled'] ?? false) ? 'active' : 'pending';
                $store->kyc_status = empty($status['requirements']['currently_due'] ?? []) ? 'verified' : 'pending';
                if ($store->kyc_status === 'verified') {
                    $store->kyc_verified_at = $store->kyc_verified_at ?? now();
                }
                $store->save();
            } catch (\Exception $e) {
                info('Stripe Connect onboarding return check failed: ' . $e->getMessage());
            }
        }
    ...
    public function stripeOnboardingRefresh(Request $request)
    {
        $store = Store::find($request->store_id);
        $accountId = $store?->gateway_account_id ?? $store?->stripe_account_id ?? null;
        ...
        if ($store && !empty($accountId)) {
            try {
                ...
                $link = $stripeService->createAccountLink(
                    $accountId,
                    route('stripe_onboarding_refresh', ['store_id' => $store->id]),
                    route('stripe_onboarding_return', ['store_id' => $store->id])
                );

                return redirect()->away($link['url']);
            } catch (\Exception $e) {
                info('Stripe Connect onboarding refresh failed: ' . $e->getMessage());
            }
        }

...
class GetBestDealsTool implements Tool
{
    /**
    ...
    {
        return 'Get items with the best discounts and current offers. Uses the platform\'s discounted scope which includes item-level discounts, store-wide offers, and flash sales. Use for "deals", "offers", "cheap options", or "what\'s on sale" queries.';
    }
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'limit'    => $schema->number()->description('Number of deals to return, default 8, or null for default')->required()->nullable(),
        ];
    }
    ...
    public function handle(Request $request): string
    {
        ...
        $this->context->recordTool('GetBestDealsTool');
        $this->context->addProducts($products);

        if (count($products) === 0) {
            return 'No discounted items found right now.';
        }

        $lines = implode('; ', array_map(function (array $p): string {
            $base = $p['name'] . ' [ID:' . $p['id'] . '] ' . $p['discount_label'] . ' now ' . $p['discounted_price'];
        ...
        return 'Best deals (' . count($products) . ' items): ' . $lines;
    }
    ...
    private function format(Item $item): array
    {
        ...
        $discountLabel = $discount > 0
            ? ($discountType === 'percent' ? $discount . '% off' : number_format($discount, 2) . ' off')
            : 'No discount';
